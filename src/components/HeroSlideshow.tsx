...
export function HeroSlideshow({ slides }: { slides: Slide[] }) {
  ...
  useEffect(() => {
    ...
    let raf = 0;
    const onScroll = () => { cancelAnimationFrame(raf); raf = requestAnimationFrame(() => { el.style.transform = `translate3d(0, ${Math.min(window.scrollY, 900) * 0.28}px, 0)`; }); };
  ...
  return (
    <div className="absolute inset-0 overflow-hidden" aria-hidden="true">
      ...
      {slides.length > 1 && (
        <div className="absolute bottom-20 right-6 z-10 flex gap-2 sm:bottom-24 sm:right-10">
          {slides.map((_, k) => (
